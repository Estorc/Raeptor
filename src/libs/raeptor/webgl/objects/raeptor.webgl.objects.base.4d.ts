...
import { RaeptorWebGLObjectsBase } from "./raeptor.webgl.objects.base";
import { mat4, vec3, vec4 } from "gl-matrix";

export class RaeptorWebGLObjectsBase4D extends RaeptorWebGLObjectsBase {
    private position : vec3 = [0, 0, 0];
    private rotation : mat4 = mat4.create();
    private scale : vec3 = [1, 1, 1];

    constructor(gl : WebGLRenderingContext, vsSourceURL : string = '/assets/shaders/raeptor.shader.4d.fancy-wireframe.vs', fsSourceURL : string = '/assets/shaders/raeptor.shader.4d.fancy-wireframe.fs') {
        super(gl, vsSourceURL, fsSourceURL);
    }

    override update(renderer : RaeptorWebGLRendererBasic) : void {
        // Update the model matrix
    }

    public override render(renderer : RaeptorWebGLRendererBasic) : void {
