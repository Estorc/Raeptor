...
class Camera {
    ...
    public pos : FeatherTypes.SmartVector3;
    public rot : FeatherTypes.SmartVector3;
	
	constructor(x : number, y : number, z : number, rotx : number, roty : number, rotz : number) {
		
		this.pos = new FeatherTypes.SmartVector3(x,y,z);
		this.rot = new FeatherTypes.SmartVector3(rotx,roty,rotz);
...
class Plane {
	...
	set(x : number, y : number, z : number, rotx : number, roty : number, rotz : number) : void {
		this.pos.x = x;
        this.pos.y = y;
        this.pos.z = z;
		this.rot.x = rotx;
...
class Point {
    ...
    public pos : FeatherTypes.SmartVector3;
    public vel : FeatherTypes.SmartVector3;
	...
	constructor(x : number, y : number, z : number) {
		...
		this.pos = new FeatherTypes.SmartVector3(x,y,z);
		this.vel = new FeatherTypes.SmartVector3(0,0,0);
		this.links = [];
		this.hasMove = false;
	...
	get2DProjection(camera : Camera, plane : Plane) : FeatherTypes.SmartVector3 | null {
		...
		return new FeatherTypes.SmartVector3(
                    plane.pos.z/d.z*d.x+plane.pos.x,
                    plane.pos.z/d.z*d.y+plane.pos.y,
                    d.z
                );
	...
	repulseFromPoint(p : Point, distance : number) : void {
...
class Segment {
	...
	get2DProjection(camera : Camera, plane : Plane) : Array<FeatherTypes.SmartVector3> | null {
		
        const PointA : FeatherTypes.SmartVector3 | null = this.points[0].get2DProjection(camera,plane);
        const PointB : FeatherTypes.SmartVector3 | null = this.points[1].get2DProjection(camera,plane);
	...
	draw(ctx : CanvasRenderingContext2D, camera : Camera, plane : Plane) : void {
	...
	trace(ctx : CanvasRenderingContext2D, camera : Camera, plane : Plane) : number {
	...
	traceFace(ctx : CanvasRenderingContext2D | any, camera : Camera, plane : Plane) : number {
...
class Face {
	...
	constructor(s1 : Segment, s2 : Segment, s3 : Segment) {
		
		this.segments = [s1,s2,s3];
        this.points = [s1.points[0],s1.points[1],s2.points[0],s2.points[1],s3.points[0],s3.points[1]];
        this.cameraDistance = 0;
		
	}
	
	trace(ctx : CanvasRenderingContext2D | any, camera : Camera, plane : Plane) : number {
...
class Mesh {
    ...
    public pos : FeatherTypes.SmartVector3;
    public rot : FeatherTypes.SmartVector3;
    public scale : FeatherTypes.SmartVector3;
	...
	constructor() {
        ...
        this.points = [];
        this.faces = [];
        this.pos = new FeatherTypes.SmartVector3(0,0,0);
        this.rot = new FeatherTypes.SmartVector3(0,0,0);
        this.scale = new FeatherTypes.SmartVector3(1,1,1);
		this.createPoints();
		this.createFaces();
		this.cameraDistance = 0;
	...
	update() : void {
		...
		this.applyVelocity();
		this.points.forEach(item => item.update());
	...
	createFaces() : void {
		...
		for (let item of this.segments) {
			...
			item.points[0]
			const segmentsA : Segment[] = this.segments.filter(a => a!=item && a.points.includes(item.points[0]));
			const segmentsB : Segment[] = this.segments.filter(a => a!=item && a.points.includes(item.points[1]));
			
			const segmentsC : Segment[] = segmentsA.concat(segmentsB);
			const segmentsD : Segment[] = [];
